...
RegisterNetEvent("adminjail:openJailMenu")
AddEventHandler("adminjail:openJailMenu", function()
	OpenJailMenu()
end)
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			if DistanceCheck <= 2.0 then
				if IsControlJustPressed(0, 38) then
					...
					TriggerServerEvent("adminjail:prisonWorkReward")
				end
			end
		end
...
function OpenJailMenu()
	...
	function(data, menu)
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
								  	...
								  	TriggerServerEvent("adminjail:jailPlayer", GetPlayerServerId(closestPlayer), jailTime, reason)
								end
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("adminjail:retrieveJailedPlayers", function(playerArray)
				...
				function(data2, menu2)
					...
					TriggerServerEvent("adminjail:unJailPlayer", action)
